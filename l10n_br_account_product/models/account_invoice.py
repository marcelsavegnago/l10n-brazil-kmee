                ...
                if invoice.nfe_version == '4.00':
                    if not invoice.account_payment_ids:
                        if invoice.fiscal_category_id and \
                            ...
                            account_payments = self.env[
                                'account.invoice.payment']

                            amount = 0.00
                            if not (invoice.fiscal_category_id.
                                    account_payment_term_id.forma_pagamento ==
                                    FORMA_PAGAMENTO_SEM_PAGAMENTO):
                                amount = invoice.amount_total
                            values = {
                                'payment_term_id':
                                    invoice.fiscal_category_id.
                                    account_payment_term_id.id,
                                'amount': amount,
                            }
                        ...
                        if not invoice.account_payment_ids:
                            raise UserError(
                                _(u'A nota fiscal deve conter dados '
                                  u'de pagamento')
                            )
                        elif invoice.amount_change < 0:
                            raise UserError(
                                _(
                                    u'O total de pagamentos deve ser maior ou'
                                    u' igual ao total da nota.\n'),
                                _(
                                    u'Resta realizar o pagamento'
                                    u' de %0.2f' % invoice.amount_change)
                            )
                    else:
        ...
        if (self.fiscal_category_id and
            ...
            for name, val in value.iteritems():
                if isinstance(val, tuple):
                    value[name] = val[0]
            values.update(value)
            self.account_payment_ids = [(0, 0, values)]
