...
class AccountAnalyticLine(osv.osv):
    ...
    def invoice_cost_create(self, cr, uid, ids, data=None, context=None):
        ...
        invoice_ids = super(AccountAnalyticLine, self).\
            invoice_cost_create(cr, uid, ids, data, context)
        for invoice in inv_obj.browse(cr, uid, invoice_ids, context=context):
            #  = line_obj.search(cr, uid, [('invoice_id', '=', invoice.id)])
            ...
            onchange = inv_obj.onchange_partner_id(cr, uid,
                                                   [invoice.id],
                                                   'out_invoice',
            ...
            for line in invoice.invoice_line:
                result = line_obj.product_id_change(
                    cr, uid, ids,
                    line.product_id.id,
                    line.uos_id.id, line.quantity, line.name,
            ...
            inv_obj.write(cr, uid, [invoice.id], onchange['value'],
                          context=context)
