...
class ConsultaDFe(models.Model):
    ...
    def busca_documentos(self):
        for consulta in self:
            ...
            # uu_obj.write({'ultimo_ns': processo.resposta.ultNSU.valor,
            # 'ultima_consulta': str(UTC.normalize(agora()))})

            ##
            # Consumo indevido,
            # não tem mais documentos para retornar, deve esperar
            # 1 hora para a próxima execução
            ##
            # if processo.resposta.cStat.valor == '656':
            # uu_obj./
            # write({'ultima_consulta': str(UTC.normalize(agora()))})
            # cron_id = self.pool./
            # get('ir.cron')./
            # search(cr, 1, [('function', '=',
            # 'acao_demorada_busca_documentos')])
            # if cron_id is not None and len(cron_id):
            # cron_obj =
            # self.pool.get('ir.cron').browse(cr, 1, cron_id[0])
            # nova_execucao =
            # parse_datetime(cron_obj.nextcall + ' GMT')
            ...
            # uu_obj./
            # write({'ultimo_ns': processo.resposta.ultNSU.valor,
            #  'ultima_consulta': str(UTC.normalize(agora()))})
            ...
            # partner_ids = partner_pool./
            # search(cr, uid, [('cnpj_cpf', '=', cnpj)])
            # if partner_ids:
    ...
    # def acao_demorada_busca_documentos(self, cr, uid, ids=[], context={}):
        ...
        # self.pool.get('sped.ultimo_ns')./
            # busca_documentos(cr, uid, ids, context)
